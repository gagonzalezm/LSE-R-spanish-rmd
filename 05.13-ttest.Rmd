...
En el capítulo anterior cubrimos la situación en la que tu variable de resultado es de escala nominal y tu variable de predicción^[No cubriremos múltiples predictores hasta el Capítulo 15 \@ref(regression)] también es de escala nominal. Muchas situaciones del mundo real tienen esa característica, por lo que descubrirás que las pruebas de chi-cuadrado en particular son ampliamente utilizadas. Sin embargo, es mucho más probable que te encuentres en la situación en la que tu variable de resultado sea de escala de intervalo o de razón, y lo que te interesa es si el valor promedio de la variable de resultado es mayor en un grupo u otro. Por ejemplo, un psicólogo podría querer saber si los niveles de ansiedad son más altos entre los padres que entre los que no lo son, o si la capacidad de la memoria de trabajo se reduce al escuchar música (en relación con no escuchar música). En un contexto médico, podríamos querer saber si un nuevo medicamento aumenta o disminuye la presión arterial. Un científico agrícola podría querer saber si agregar fósforo a las plantas nativas australianas las matará.^[La experimentación informal en mi jardín sugiere que sí, lo hace. Las nativas australianas están adaptadas a bajos niveles de fósforo en comparación con cualquier otro lugar de la Tierra, aparentemente, por lo que si compró una casa con un montón de plantas exóticas y deseas plantar nativas, no sigas mi ejemplo: mantenlas separados. Los nutrientes para las plantas europeas son venenos para las australianas. Probablemente hay una broma en todo esto, pero no puedo saber si lo es.]  En todas estas situaciones, nuestra variable de resultado es una variable realmente continua, de intervalo o de razón; y nuestra predictora es una variable binaria de "agrupamiento". En otras palabras, queremos comparar las medias de los dos grupos.
...
knitr::include_graphics(file.path(projecthome, "img/ttest2/tdist_3.eps"))
...
knitr::include_graphics(file.path(projecthome, "img/ttest2/ttestci.eps"))
...
knitr::include_graphics(file.path(projecthome, "img/ttest2/shapirowilkdist.eps"))
