...
# Una breve introducción a diseños de investigación{#studydesign}

>*Consultar al estadístico después de que un experimento ha finalizado es a menudo 
...
La primera cosa para entender la recolección de datos puede ser pensada como un tipo de **medición.** Esto es, lo que estamos intentando hacer
aquí es medir algo sobre la conducta humana o la mente humana. ¿Qué quiero decir por "medición"?

### Algunos pensamientos sobre medición psicológica
...
bien definidas, a "cosas". Entonces, alguno de los siguientes ejemplos contaría como una medición psicológica:
 ...
 -Mi **género auto-identificado** es *masculino* ^[Bien... ahora esto incómodo ¿no? Esta sección del libro es una de las partes más antiguas del libro, y está desactualizada en una manera
 ...
 era entonces y es ahora XY. El genero auto-identificado, por otro lado...ah. Supongo que el hecho de que el titulo refiera a mi como Danielle 
 ...
 En breve la lista anterior, la **parte enegrecida** es la cosa a ser "medida", y la *parte en cursiva* es "la medición en sí misma". De hecho,
 podemos expandir esto un poco más, pensando sobre un de posibles mediciones que podrían haber aparecido en cada caso: 
 
  -Mi **edad** (en años) podría haber sido *0,1,2,3...,etc. El limite superior que mi edad podría ser podría ser un poco difuso, peor en la práctica
  tu estarías seguro diciendo que la edad más alta posible es *150*, desde que ningun humano ha vivido tanto. 
  
  -Cuando se me preguntó si **me gustan las anchoas**, podría haber dicho, *Sí, o No, o No tengo opinion, o A veces*
  
  -Mi **género cromosómico** casi ciertamente iba a ser *masculino (XY) o femenino (XX)*, pero hay unas pocas otras posibilidades- Podría tener el *Sindrome de
  Klinfelter (XXY), el cuál es más similar a masculino que a feminino. E imagino hay otras posibilidad es tambien. 
  
  -Mi **genero auto-identificado** es tambien casi muy probablemente *masculino* o *femenino*, pero no tiene que concordar con mi género cromosómico.
  Podría tambien escoger identificar con *ninguno* o explicitamente identificarme como *transgénero*. 
...
Todas las ideas discutidas en la sección anterior se relacionan con el concepto de ***operación***. Para ser un poco más precisos sobre la idea, la operacionalización es el proceso por el cual tomamos un concepto significativo pero algo vago, y lo convertimos en una medida precisa. El proceso de operacionalización puede implicar varias cosas diferentes:
...
- Una ***construcción teórica***. Esto es lo que intentas medir, como la "edad", el "género" o una "opinión". Un constructo teórico no puede ser observado directamente, y a menudo son en realidad un poco vagos. 
- Una ***medida***... La medida se refiere al método o la herramienta que se utiliza para hacer las observaciones. Una pregunta en una encuesta, una observación del comportamiento o un escáner cerebral podría contar como una medida. 
- Una ***operacionalización***. El término "operacionalización" se refiere a la conexión lógica entre la medida y el constructo teórico, o al proceso por el cual intentamos derivar una medida de un constructo teórico.
- ***Una variable***. Finalmente, un nuevo término. Una variable es lo que obtenemos cuando aplicamos nuestra medida a algo en el mundo. Es decir, las variables son los "datos" reales con los que terminamos en nuestros conjuntos de datos.
